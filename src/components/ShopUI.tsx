...
import { X, Minus, Square, ShoppingCart, Search, Globe, ChevronLeft, ChevronRight, RotateCw, LayoutDashboard } from 'lucide-react';
...
const ProductImage = ({ src, alt }: { src: string; alt: string }) => {
  ...
  if (isPicsum) {
    ...
    if (match) {
      const seed = match[1];
      srcSet = `https://picsum.photos/seed/${seed}/400/400 400w, https://picsum.photos/seed/${seed}/800/800 800w`;
    }
  }

  return (
    <picture className="w-full h-full block">
      {srcSet && <source srcSet={srcSet} sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />}
      <img 
        src={src} 
        alt={alt}
        loading="lazy"
        decoding="async"
        className={`w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ${!isGif ? 'mix-blend-multiply' : ''}`}
    ...
    >
      ...
      <div className="w-full max-w-6xl h-full sm:max-h-[900px] bg-[#111] sm:rounded-2xl sm:border-4 border-[#333] shadow-2xl shadow-cyan-500/20 flex flex-col overflow-hidden relative">
        ...
        <div className="hidden sm:flex h-6 w-full bg-[#0a0a0a] border-b border-[#222] items-center justify-center">
          <div className="w-2 h-2 rounded-full bg-[#222]"></div> {/* Webcam */}
        </div>
            ...
            <div className="bg-[#e2e2e2] border-b border-gray-300 flex flex-col">
              ...
              <div className="h-12 sm:h-10 flex items-center px-4 justify-between">
                <div className="flex items-center gap-3 sm:gap-2">
                  <button onClick={onClose} className="w-5 h-5 sm:w-3.5 sm:h-3.5 rounded-full bg-red-500 hover:bg-red-600 flex items-center justify-center group" title="Close">
